...
function aiovideodl(link) {
  return new Promise((resolve, reject) => {
    axios({
      ...
      axios({
        url: "https://aiodown.com/wp-json/aio-dl/video-data/",
        method: "POST",
        headers: {
          "user-agent":
            "Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/91.0.4472.124 Safari/537.36",
...
const snapsave = async (url) => {
  const { data } = await axios.post(
    ...
    encodeDom = data.split(/<script type=".+?">(.*?)<\/script>/)[1],
    doms = encodeDom.split("}(")[1].replace(/"/g, "").split(")")[0]?.split(","),
    decoded = decode(...doms)
      .split("</style>")[1]
      .split('";')[0];
  let list = [
    ...new JSDOM(decoded).window.document.querySelectorAll(
      "table > tbody > tr",
    ),
  ].filter((x) => x.querySelector("td > a"));
  return {
...
async function yt(url, quality, type, bitrate, server = "en68") {
  ...
  let list,
    res = await post(`https://www.y2mate.com/mates/${server}/analyze/ajax`, {
      url: url,
      q_auto: 0,
      ajax: 1,
    }),
    json = await res.json(),
    { document } = new JSDOM(json.result).window,
    table =
